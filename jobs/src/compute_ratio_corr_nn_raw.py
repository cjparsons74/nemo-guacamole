...
def compute_render_ratio_corr(
    ...
    for roi_idx in range(
        ...
        plt.scatter(
            ...
            * len(np.array(nncorr_dict[roi_idx])) 
        )
    ...
    plt.savefig(
        f"{sdir}Rawratiocorrelations_{tag}_ROIs:{num_rois}_NN:{nnpop}_seed:{seed}_RN:{rnpop}.png",
    )
    plt.close()
...
def main():
    num_rois = 20000
    seed = None
    sdir = "/camp/home/duuta/working/duuta/jobs/plots/ratioCorr/raw/rawRC20K/"

    for nnpop, rnpop in zip([30, 60, 80, 100, 1000], [30, 60, 80, 100, 1000]):
